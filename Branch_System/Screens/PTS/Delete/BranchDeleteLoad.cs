...
{
    public partial class BranchDeleteLoad : MaterialSkin.Controls.MaterialForm
    {
        ...
        private void Record_DGView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if(isBranchAdmin)
            {
                ...
                LoadBranchDeleteScreen delete = new LoadBranchDeleteScreen();
                if (records != null)
                {
                    ...
                    if (q.Count() > 0)
                    {
                        ...
                        this.Update();
                        delete.record = q[0];
                        delete.Closed += (s, args) => { this.GetUnAuthRecords();
